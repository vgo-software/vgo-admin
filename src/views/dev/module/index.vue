  ...
  <div class="table-box">
    <ProTable ref="proTable" :columns="columns" :request-api="getTableList" :init-param="initParam" :data-callback="dataCallback">
      ...
      <template #tableHeader>
        <el-button v-auth="'create'" type="primary" :icon="CirclePlus" @click="openDrawer('创建模块')">创建模块</el-button>
    ...
    <ModuleFormDrawer ref="drawerRef" />
  </div>
</template>

<script setup lang="tsx" name="module">
...
import { getList, deleteInfo, exportInfo, addInfo, editInfo, changeInfoStatus } from "@/api/modules/module";
import ModuleFormDrawer from "@/views/dev/module/FormDrawer.vue";
...
// 切换用户状态
const changeStatus = async (row: Module.ResList) => {
  await useHandleData(changeInfoStatus, { id: row.id, status: row.status == 1 ? 2 : 1 }, `切换【${row.title}】状态`);
  proTable.value?.getTableList();
};

// 表格配置项
const columns = reactive<ColumnProps<Module.ResList>[]>([
  {
    prop: "name",
    label: "名称",
    render: scope => {
...
// 打开 drawer(新增、查看、编辑)
const drawerRef = ref<InstanceType<typeof ModuleFormDrawer> | null>(null);
const openDrawer = (title: string, row: Partial<Module.ResList> = {}) => {
  const params = {
    title,
    isView: title === "查看",
    row: { ...row },
...
// 删除信息
const deleteAccount = async (params: Module.ResList) => {
